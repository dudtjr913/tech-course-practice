...
## 구현해야 할 기능
  - 메인화면, 기능선택, 테이블목록, 테이블 선택, 등록할 메뉴, 메뉴의 수량, 주문 내역, 결제 진행, 결제 수단, 결제 금액을 class로 만든다.
  - 전역 변수로 테이블 번호, 메뉴 이름, 가격, 수량을 등록할 수 있는 객체를 만든다. **order**
  - 사용자의 기능 선택을 받을 함수를 만든다. **submitMainOption**
  - 기능 선택을 받으면, 그 기능이 유효한지 체크하는 함수를 만든다. **isValidMainInput**
  - 1번을 선택한다면 테이블 목록과 테이블 선택을 할 수 있는 함수를 호출한다. **selectTable**
  - 테이블을 선택한다면, 그 기능이 유효한지 체크하는 함수를 만든다. **isValidTableInput**
  - 올바르다면 메뉴를 선택할 수 있는 함수를 호출한다. **selectMenu**
  - 메뉴 선택을 받고, 메뉴 선택이 유효한지 체크하는 함수를 만든다. **isValidMenuInput**
  - 올바르다면 메뉴 수량을 입력받는다. **selectMenuAmount**
  - 메뉴 수량이 유효한지 체크하는 함수를 만든다. **isValidMenuAmountInput**
  - 다시 submitMainOption을 호출해서 위의 기능을 반복한다. 
  - 2번을 누르면 주문 내역을 보여주는 함수를 만든다. - **showUserOrderMenu**
  - 결제 진행을 하는 함수를 만든다. **makePayment**
  - 결제 수단을 입력받고, 유효성 검사를 하는 함수를 만든다. **isValidPaymentInput**
  - 최종 결제할 금액을 알려주는 함수를 만든다. **showPriceToUser**
  - 할인을 진행하는 함수를 만든다. **makeDiscount**
  - submitMainOption으로 돌아가서 반복한다.
  - 3번을 입력할 시 종료하는 함수를 만든다. **finishGame**
