...
- [v] 횟수 입력을 받을 함수를 구현한다.
  ...
  - [v] 횟수 입력의 유효성을 확인하는 함수를 구현한다.
    - 예외처리)
      ...
      - [v] 0을 이하의 수를 입력했을 때
      - [v] 빈칸을 입력했을 때
- [v] RacingCar class에 횟수만큼 반복문을 돌면서 게임을 진행할 메소드를 구현한다.
- [v] 자동차를 움직이는 함수를 구현한다.(4보다 크면 움직이고, 아니면 정지)
- [v] 횟수 입력이 유효하다면 위의 메소드에 input을 인자로 주면서 호출한다.
- [v] 실행 결과 창을 display : block으로 만들어주는 함수를 구현한다.
- [v] 실행 결과를 보여줄 함수를 구현한다.
  - [v] 진행사항을 보여줄 함수를 구현한다.
  - [v] 우승자를 보여줄 함수를 구현한다.
    - [v] 우승자를 가려낼 함수를 구현한다.
